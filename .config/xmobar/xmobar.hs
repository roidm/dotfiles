...
Config { font    = "xft:UbuntuMono Nerd Font:weight=bold:pixelsize=20:antialias=true:hinting=true"
       ...
       , commands = [
                    ...
                    , Run Com "~/.local/bin/dwmpulse" ["--listen"] "" 10
                    , Run MultiCoreTemp ["-t", "﨎 <avg>°C", "-hwmonitor-path", "/sys/module/k10temp/drivers/pci:k10temp/0000:00:18.3/hwmon/hwmon2"] 50
                    , Run UnsafeStdinReader
                    ]
       , sepChar = "%"
       , alignSep = "}{"
       , template ="<fc=#bc7ad9>   </fc> %UnsafeStdinReader% }{<fc=#e06c75> %enp34s0% </fc><fc=#4b5363>|</fc><fc=#6bb2c0> %multicoretemp% </fc><fc=#4b5363>|</fc><fc=#bc7ad9>   %memory%</fc><fc=#4b5363>|</fc><fc=#ebcb8d>%cpu% </fc><fc=#4b5363>|</fc><fc=#71abeb> %dwmpulse% </fc><fc=#4b5363>|</fc><fc=#9ec07c> %date% </fc>"
       }
